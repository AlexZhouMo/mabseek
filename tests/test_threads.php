...
check(thread_valid_category('method') === true, 'category 合法');
...
$tid = thread_create($uid, 'method', '  我的第一帖  ', "第一行\n第二行", $okCover);
...
$tid2 = thread_create($uid, 'resource', $injTitle, 'body-or-1-eq-1', $okCover);
check(thread_get($tid2)['title'] === $injTitle, '注入串原样入库、参数化生效');

// ── 分类分页查询 ──
// 现有：$tid(method) + $tid2(resource)。再造 2 条 resource 便于分页验证。
$b1 = thread_create($uid, 'resource', 'resource 帖一', '正文', $okCover);
$b2 = thread_create($uid, 'resource', 'resource 帖二', '正文', $okCover);
$allRows = thread_list_by_category('all', 50, 0);
check(count($allRows) >= 4, 'category=all 返回全部（≥4）');
$bioRows = thread_list_by_category('resource', 50, 0);
$bioIds  = array_column($bioRows, 'id');
check(count($bioRows) === 3 && in_array($tid2, $bioIds, true) && in_array($b1, $bioIds, true) && in_array($b2, $bioIds, true), 'category=resource 只回 3 条 resource 帖');
check(!in_array($tid, $bioIds, true), 'category=resource 不含 method 帖');
// 分页：resource 每页 2 条
$page1 = thread_list_by_category('resource', 2, 0);
$page2 = thread_list_by_category('resource', 2, 2);
check(count($page1) === 2 && count($page2) === 1, 'resource 分页：首页 2 条、次页 1 条');
...
$editTid = thread_create($oid, 'method', '原标题', '原正文', $okCover);
check(thread_update($editTid, $other, 'method', '篡改', '篡改', $okCover) === false, '非作者更新被拒');
check(thread_get($editTid)['title'] === '原标题', '数据未被越权修改');
check(thread_update($editTid, $oid, 'resource', '新标题', '新正文', $okCover) === true, '作者本人更新成功');
...
// ── 后台治理 + 级联删除 ──
$aTid = thread_create($oid, 'method', '待治理帖', '正文', $okCover);
...
// 级联：删帖主 → 其帖随外键消失
$cTid = thread_create($oid, 'resource', '级联测试', '正文', $okCover);
